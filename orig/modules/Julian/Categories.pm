...
# including relationships between items and categories
#
################################################
...
@Categories::structure =
 ...
 (
   ...
   "CREATE TABLE %%name%%$Categories::structure_tables[2]

   (
	ID1	BIGINT(1) PRIMARY KEY,
	ID2	BIGINT(1),
	RELATE	VARCHAR(16),	# use human-readable relationship code, or 16-byte 'catcode' from CatTree.pm
	KEY	index_1 (ID2)
   )
   "
...

{
 ...
 if($type =~ m/^ITEM/si)

  {
   ...
   $q = "DELETE FROM ".$self->name().$cats_s_table[2]." WHERE (ID1=$q_id) OR (ID2 = $q_id)";
...

{
 ...
 if($id != 0)

  {
   ...
   $q = "DELETE FROM ".$self->name().$cats_s_table[2]." WHERE (ID1=$qCID) OR (ID2 = $qCID)";
...
# TODO: delete relationships of these items
# what if item belonged to other categories as well?  Delete anyway? 
...

{
 ...
 $q = "DELETE FROM ".$self->name().$cats_s_table[2];
...

{
 ...
 my $id1		= $inp{'id1'}       || undef;  # ID of first Item/Category in relationship

 my $id2	      	= $inp{'id2'}	    || 0;  # ID of second Item/Category, 0 for external relation 

 my $relate		= $inp{'relate'}    || 'EQUAL'; # Relationship: 'EQUAL', 'CONTAINS', 'ISKINDOF','SIMILAR', others.  TODO provide list.  
							# For external relationships, may be index code
 ...
 if($id1 eq undef)

  {
   ...
   $self->error("Can't RELATE Item/Category without ID!");
 ...
 my $q_relate = $dbh->quote($relate);

 $q = "INSERT INTO ".$self->name().$cats_s_table[2]." SET ID1=$id1, ID2=$id2, RELATE=$q_relate";
...

{
 ...
 my $relate		= $inp{'relate'}    || 'EQUAL';

 my $all		= $inp{'all'}	    || 'N';	# Delete all relationships of ID1
 ...
 if($all eq 'Y')

  {
   ...
   $q = "DELETE FROM ".$self->name().$cats_s_table[2]." WHERE ID1=$id1";

  }
else
  {
   $q = "DELETE FROM ".$self->name().$cats_s_table[2]." WHERE (ID1=$id1) AND (ID2=$id2) AND (RELATE = $q_relate)";
  }
...

{
 ...
 my $relate = $inp{'relate'} || 'ANY';

 my $filter          = $inp{'filter'}             || 'ALL';   # Items or categories

 								# applicables are: 'ITEMS','ALL','CATEGORIES'

 my $multiple        = $inp{'multiple'}           || 'Y';       # Return many rows of results.

 my $by              = $inp{'by'}                 || 'ID1';      # Search by 'ID1', applicables are: 
 ...
 if($dbh)

  {
   ...
   $where = " WHERE (".$self->name().$cats_s_table[2].".$by = $id) AND ";
   if($filter =~ m/^(ITEMS|ALL)$/si)

     {

      $where .= "(".$self->name().$cats_s_table[1].".ID = ".$self->name().$cats_s_table[2].".$find_by) "; 

      my $q = "SELECT * FROM ".$self->name().$cats_s_table[1].",".$self->name().$cats_s_table[2].$where.$order.$limits.$rev;
   ...
   if($filter =~ m/^(CATEGORIES|ALL)$/si)
	...
	if ($filter =~ m/^ALL$/si) {
      ...
      $where .= "(".$self->name().$cats_s_table[0].".CID = ".$self->name().$cats_s_table[2].".$find_by) "; 

      my $q = "SELECT * FROM ".$self->name().$cats_s_table[0].",".$self->name().$cats_s_table[2].$where.$order.$limits.$rev;
...
# TODO: if this works, extract out rest of this routine as subroutine, call here and
# from find().  Otherwise is duplicate code.

...
$q = "select * from rcatdb_categories where ((REL_URL is NULL) OR (REL_URL = '') OR (REL_URL LIKE '% ') OR (REL_URL LIKE '% /%')) order by id";
...
while ($ref = $sth->fetchrow_hashref('NAME_uc')) {
	...
	my $q1 = "update rcatdb_categories set REL_URL = ".$dbh->quote($relurl)." where ID = $catid ";
	$dbh->do($q1);

...
# ex: ./import_dmoz_subcat_tree.pl 66906 '2:10:9' 'Education Categories (ODP)'
...
$usage = "Usage: import_dmoz_subcat_tree.pl [parent_cat_id] [structure_catcode as N:M:O..] [newcatname]\n";
$parentcat = $ARGV[0] || die($usage);
$struct_code = $ARGV[1] || die($usage);
$newcatname = $ARGV[2] || '';

my $catcode = &AbUtils::catcode_from_str($struct_code);
my $qcatcode = $dbh->quote($catcode);
my ($struct_id) = $dbh->selectrow_array("select catid from structure where catcode = $qcatcode");
$struct_id || die("Canot find dmoz cat with id $struct_id\n");

%CAT_ID = ();


my $dmoz_lvl = &AbUtils::GetLevelfromCatcode($catcode) +1;


$dmoz_lvl || die("No level from catcode structure id $struct_id");
...
print "inserting under category $parent_catcode_str\n";


$CAT_ID{$parent_catcode} = $parentcat;

#$q = "select * from rcatdb_categories where id = 306 or cid = 30 or cid = 306";
$q = "select catid, catcode, title,lastsubcode from structure where LEFT(catcode, $dmoz_lvl) = LEFT($qcatcode, $dmoz_lvl) $LIMIT";

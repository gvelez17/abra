...
# do NOT use my - modifies global exported from AbHeader
$DBNAME = 'rcats';
$DBUSER = 'rcats';
$DBPASS = 'meoow';
...
$LOG_FILE = '/home/sites/abra/logs/qlog';
...
if ($ENV{'SERVER_NAME'} =~ /btucson.com/i) {
	...
	$ADD_TEMPLATE_FILE = 'tmplAddURL.html';  # need add form too
	$DEFAULT_CAT = 76680;

# we were going to use a separate db, but no longer
#        $DBNAME = 'ab_recipes';
#        $DBUSER = 'rcats';
#        $DBPASS = 'meoow';
} elsif (($ENV{'SERVER_NAME'} =~ /tech-for-teachers.com/i) || ($ENV{'SERVER_NAME'} =~ /teach/i)) {
	$TEMPLATE_DIR = '/home/sites/teachtech/itempages';
	$TEMPLATE_FILE = 'tmplAddURL.html';
	$ADD_TEMPLATE_FILE = 'tmplAddURL.html';
	$DEFAULT_CAT = 66855;
}
...
$q = "select ID,CID,NAME from rcatdb_items where url = ".$dbh->quote($obj_url)." limit $MAX_RECORDS";
...
$templatehash->{CID} =$catid || $DEFAULT_CAT;

...
if (! $username) {
	...
	if ($anon_ok) {
		...
		&ErrorExit("Sorry, you must be logged in to perform posts.  Please press the back arrow to return to the page you were at and look for  the 'login' link, or use this <A HREF='http://abra.btucson.com/php/access_user/login.php'>generic login</A> page\n");
	}
}	
...
$debug && print "cat is $cat, catid is $catid, catstring is $catstring, catcodestr is $catcodestr";
if ($catid) {
	$cat = $catid;
} elsif ($catcodestr) {
...
$debug && print "..got code $code and catid $cat\n, converts back to ".&AbUtils::catcodestr($code)."\n<br>";
} elsif($catstring) {
	$cat = ParseCatString($catstring);
} 

if (! $code) {
	$code = &AbUtils::get_catcode($cat);
}

$debug && print(" catstring is $catstring Cat is $cat");

# TODO here add cat if possible
if ( $code &&  ! $cat ) {
	if (&ab_dmoz_catcodes::is_dmoz_catcode($code)) {

		$cat = &ab_dmoz_catcodes::create_tree_for_dmoz_catcode($code);
	}
	$cat || &ErrorExit("Required field _catid or catcode not found, or category $catstring not found");
}
...
sub ErrorExit {
	...
	print "Got inputs ",%in;
	print 'Please <A HREF="http://webglimpse.net/contact.php">Contact us</A> for assistance.';
	print "</font></html>\n";
	exit(1);
}	

...
# TODO 10/14/07 use /home/sites/abra/templates/tmplRelateToURL.html
...
$DEFAULT_TEMPLATE_FILE = 'tmplRelateToURL.html';
...
if ($ENV{'SERVER_NAME'} =~ /btucson.com/i) {
        $TEMPLATE_DIR = '/home/sites/iwtucson/itempages';
        $TEMPLATE_FILE = 'tmplAddURL.html';  # currently we always add to bTucson

} elsif (($ENV{'SERVER_NAME'} =~ /myrecipesearcher.com/i) || ($ENV{'SERVER_NAME'} =~ /bcookin.com/i)) {
	$DBNAME = 'ab_recipes';
	$DBUSER = 'rcats';
	$DBPASS = 'meoow';

        $TEMPLATE_DIR = '/home/sites/bcookin/templates';
        $TEMPLATE_FILE = 'tmplComment.html';
...
if (($req_url =~ /\/comments\/(.+)$/) || ($req_url =~ /\/make_a_comment\/(.+)$/)) {
	$obj_url = $1;
}
...
$q = "select ID,NAME from rcatdb_items where url = ".$dbh->quote($obj_url);
...
#print "Query was $q, where is $where";
my @items;
my $href;
...
if (! $about_id) {
	my $catid = $DEFAULT_CAT;
	my $domain = $obj_url;
	if ($domain =~ /http:\/\/(.+)$/) {
	       	$domain = $1;
	}

	my $title = $domain;  #TODO pull from site itself <TITLE> tag
	my $owner = $ADMINUSER; #TODO recognize logins

	$about_id = &AbUtils::add_item(item_owner=>$owner, security_level=>0,url=>$obj_url,itemname=>$title,cid=>$catid);
...
if ($where) {
	...
	print "I've heard of $obj_url as $name...<br>";
	print "<div about='$obj_url'>";

	# select direct relations
	my $q1 = "select RELATION, NAME, SHORT_CONTENT, URL from rcatdb_items, rcatdb_ritems where ($where) AND rcatdb_items.id = rcatdb_ritems.item_dest";
...
#print "Next query was $q1, got ".$#items." results";
	...
	for my $href (@items) {
		...
		if ($href->{URL}) {
			$linkstart = "<A HREF='".$href->{URL}."' rel='abra:".$href->{RELATION}."'>";
			$linkend = "</A>";
		}
		$relname = $RELATIONS{$href->{RELATION}}->{'NICENAME'} || $href->{RELATION};	
		print "<div>$relname: $linkstart",$href->{NAME},"$linkend<br>\n",$href->{SHORT_CONTENT},"</div>";

	}
	if ($#items < 0) {
		print "but I don't have any comments on it yet. You can add one below!";
	}
}

# print a form for people to submit comments and categorize it if not already
# action must both add the obj url as an item if not present and submit a comment on it

#TODO should check dmoz list for existing category

# Below here is stuff for querying Revyu.com.  We're not doing that for now.
